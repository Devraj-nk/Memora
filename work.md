...
## 2026-09-16 (cont. 2) — Remaining memory-quality metrics: freshness, duplicates, answer grounding, and a synthetic retrieval-recall eval

Design questions from the previous entry's "next steps" resolved before building, since they weren't obvious calls to make unilaterally:

- **Retrieval relevance/recall** - no labeled query set exists for a personal KB, so the ground truth is manufactured: for each of a sampled set of chunks, ask the LLM to generate a question that chunk answers, then check whether retrieval finds that same chunk again for its own question. Optimistic by construction (the question is generated directly from the chunk, easier than a real query) - a regression signal over time, not an absolute score.
- **Answer grounding** - a dedicated small NLI (entailment) model, not the same local LLM judging its own answer (same "small specialized model" reasoning as the reranker's cross-encoder).
- **Source quality** - left undefined; the README gives no concrete rubric, so skipped rather than guessed at.

New pieces:

- [`ingestion/chunker.py`](src/memora/ingestion/chunker.py) / [`memory/vector_store.py`](src/memora/memory/vector_store.py): `Chunk` gained `modified_at: datetime | None = None` (default `None` so every existing hand-built `Chunk(...)` in tests still works unchanged), populated by `chunker.chunk()` from `ParsedDocument.modified_at`. `VectorStore` persists it as an ISO string (same convention `observability/store.py` already uses for `created_at` - side-steps LanceDB/Arrow schema-inference edge cases around `None` timestamps). Added `all_chunks_with_vectors()` alongside the existing `all_chunks()` (which now just discards the vector half) so duplicate detection can reuse already-stored embeddings instead of recomputing them.
  - This changed the LanceDB row schema, so the local `data/db/vectors.lance` from earlier manual verification runs no longer matched and was deleted (disposable dev data, gitignored, recreated on next ingest - not a migration-worthy concern at this stage).
- [`observability/grounding.py`](src/memora/observability/grounding.py) (new): `is_grounded(context, query, answer)` using `cross-encoder/nli-deberta-v3-xsmall` (config: `settings.grounding_model`) - small enough for the CPU-only laptop this runs on, same size-consciousness as the earlier `llama3.2:3b` pick. **Finding from testing against the real model, not assumed**: a bare fragment answer like `"LanceDB."` (exactly the terse style `llama3.2:3b` tends to answer in) got misclassified as `"contradiction"` by the NLI model regardless of whether it was actually correct - the model expects full-sentence hypotheses and a noun-phrase fragment is out of distribution for it. Prepending the query to the answer (`f"{query} {answer}"`) as the hypothesis fixed this, verified with both a correct and an incorrect fragment answer against the real model before locking in the signature. Empty context short-circuits to `False` without a model call (no context genuinely can't ground anything).
- [`observability/evaluation.py`](src/memora/observability/evaluation.py) (new): `evaluate_retrieval(store, top_k, sample_size, seed)` implementing the synthetic self-check above; `generate_synthetic_question(text)` is the one-LLM-call-per-chunk piece, kept as its own monkeypatchable function so evaluation logic (sampling + hit-counting) can be tested independently of both retrieval quality and LLM behavior.
- [`observability/metrics.py`](src/memora/observability/metrics.py): `Metrics` gained `answer_grounding_rate`, `avg_memory_age_days` (`None` when no chunk has a timestamp), and `duplicate_chunk_pairs`. `compute_metrics` now takes both stores (`ObservabilityStore` and `VectorStore`) instead of just the former. Grounding is checked lazily here (once per stored trace, reusing its already-saved `context`/`query`/`response`) rather than at query time, so `POST /query/` never pays NLI latency. Duplicate detection is a numpy cosine-similarity matrix over `all_chunks_with_vectors()` (vectors already L2-normalized by the embedder, so similarity is just a dot product) above `DUPLICATE_SIMILARITY_THRESHOLD = 0.95` - documented as O(n²) and fine at personal-corpus scale, not something that scales further without a proper nearest-neighbor index.
- [`api/routes/observability.py`](src/memora/api/routes/observability.py): `GET /metrics` now also injects `get_vector_store`. Added `POST /evaluate` (body: optional `top_k`/`sample_size`) running `evaluate_retrieval` - a POST rather than GET since it triggers real LLM calls and meaningful compute, not just a read.

**Verified against live infrastructure** (real Ollama + the real NLI model, not the mocked test suite): ran the same ingest → query round trip as the previous milestone, then `compute_metrics()` and `evaluate_retrieval()` against the real stores. First run surfaced the fragment-answer grounding bug above (`answer_grounding_rate=0.0` on a genuinely-grounded answer); after the query-prefix fix, re-ran and got `answer_grounding_rate=1.0`, `avg_memory_age_days` a small positive number, `duplicate_chunk_pairs=0`, and `evaluate_retrieval` correctly recalling its one sampled chunk.

Tests: [`tests/test_grounding.py`](tests/test_grounding.py) and [`tests/test_evaluation.py`](tests/test_evaluation.py) (new), plus new cases in `test_vector_store.py` (`modified_at` round-trip, `all_chunks_with_vectors`) and a rewritten `test_metrics.py` (new fields, `is_grounded` stubbed via monkeypatch so these stay fast) and `test_observability_route.py` (`/metrics` now overrides `get_vector_store` too; new `/evaluate` case). Added a `slow` pytest marker (registered in `pyproject.toml`) for the two grounding tests that exercise the real NLI model rather than a mock. Full suite: 91/91 passing.

**Next steps:** a basic UI or CLI for `POST /ingest`, `POST /query`, and `GET /observability/*` - everything so far has only been exercised through pytest and one-off scripts, never a real interactive session. That's the natural next step now that the full pipeline (ingestion, hybrid+graph retrieval, generation, tracing, and quality metrics) is implemented end to end.

## 2026-09-16 (cont. 3) — CLI client

Chose CLI over a web UI: no new frontend stack to decide on, fits a local-first personal tool better than a browser page would.

[`cli.py`](src/memora/cli.py): a `memora` command (`ingest <path>`, `query <text>`, `trace <trace_id>`, `metrics`, `evaluate [--top-k] [--sample-size]`) that talks HTTP to a running server via `httpx` - deliberately not importing the pipeline directly, so it's a real client of what's actually deployed (request validation, dependency wiring, error responses included) rather than another one-off script bypassing the API layer. Registered as a proper console-script entry point (`[project.scripts]` in `pyproject.toml`, `pip install -e .`), so it runs as plain `memora ingest ...` once installed, not just `python -m memora.cli`. `httpx` promoted from a dev-only dependency to core, since the CLI is a real feature now, not just test tooling. New `settings.api_base_url` (default `http://localhost:8000`) tells the CLI where to find the server - same pattern as `ollama_host`. Connection failures get a specific "is the server running?" message rather than a raw traceback; HTTP error responses print the server's `detail` message and exit non-zero.

**Verified against a real running server** (`uvicorn memora.api.main:app`), not just the mocked test suite - and this surfaced a second real grounding bug beyond the fragment-answer one from the previous entry:

`memora ingest`, `query`, `trace`, and `evaluate` all worked correctly first try. `memora metrics` didn't: `answer_grounding_rate: 0.0` on a trace that was genuinely well-grounded (`query="What vector store does Memora use?"`, `response="LanceDB"`). Traced it to `context_builder.build_context()`'s `"[source]\n"` prefix - fine for the LLM prompt, but confirmed against the real NLI model that the same context+answer pair that correctly scored "entailment" dropped to "neutral" once a source path like `"[C:\...\note.md]\n"` was prepended to it; the model has never seen anything resembling a file path in training and it dilutes the premise. Fixed in [`observability/grounding.py`](src/memora/observability/grounding.py) by stripping any `"[...]\n"` header line before building the NLI premise. Caught this specifically *because* the CLI drives the real deployed path end to end (real `context_builder` output, real stored trace) rather than a hand-written test string that happened not to include the header - exactly the gap this CLI work was meant to close. Also hit a mechanical snag worth noting: the first fix attempt appeared not to work because the already-running `uvicorn` server (started without `--reload`) was still serving the pre-fix module code from memory - restarting it was necessary before the CLI could show the corrected number.

Tests: [`tests/test_cli.py`](tests/test_cli.py) (new) - uses `httpx.MockTransport` to fake server responses, so these test argument parsing and output formatting without needing a live server; covers all five subcommands plus the HTTP-error and connection-error paths. New regression case in `test_grounding.py` for the source-header stripping. Full suite: 100/100 passing.
